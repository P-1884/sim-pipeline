...
def simulate_image(
    lens_class, band, num_pix, add_noise=True, observatory="LSST",return_noise=False, **kwargs
):
    ...
    if add_noise:
        noise = sim_api.noise_for_model(model=image)
        image += noise
    if return_noise: return image,noise
    else: return image
